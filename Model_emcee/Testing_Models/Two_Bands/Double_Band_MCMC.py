...
ndim, nwalkers, points = 6, 400, 400
...
sb = np.array([sb1, sb2])
size = np.array([guess_sizeB1, guess_sizeB2])
lat = np.array([guess_latB1, guess_latB2])
phase = np.array([guess_phaseB1, guess_phaseB2])
relBright = np.array([guess_relBrightB1, guess_relBrightB2])
relVel = np.array([guess_relVelB1, guess_relVelB2])
...
outputTable = SpotBandModDouble.spotband(guess_table, bodyDiam, rotPer, incl, timeStep, numDays)
...
nll = lambda *args: SpotBandModDouble.lnlike(*args)
...
sampler = emcee.EnsembleSampler(nwalkers, ndim, SpotBandModDouble.lnprob, args = (flux, fluxerr, modTable, bodyDiam, rotPer, incl, timeStep, numDays))
...
fig = corner.corner(samples, labels=["$Band 1 Velocity$", "$Band 2 Velocity$", "$Band 1 Brightness$", "$Band 2 Brightness$", "$Band 1 Phase$", "$Band 2 Phase$"], quantiles = [0.16, 0.5, 0.84], show_titles=True, labels_args={"fontsize":40}, title_fmt=".3f")

fig.savefig("Doublecorner.png")
